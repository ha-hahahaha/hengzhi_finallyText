$(document).ready(function() {
	...
	$(".addsort").hide();
	$(".changesort").hide();
	$("#sort").css('background-color', 'rgb(255, 255, 255, 0.3)');
...
//		},
		success: function(data) {
			for(var i=0;i<data.length;i++){
				$("select").append("<option value='"+data[i].sortId+"'>"+data[i].sortName+"</option>");
			}
		},
		error: function() {
			var error = "查询数据失败！";
	...
	var sortid;
//	点击添加分类
	$('body').on('click', '.addsort2', function() {
		$(".sort").hide();
		$(".addsort").show();
		$(".changesort").hide();
	})
...
//	点击改变分类
	$('body').on('click', '#alter', function() {
		...
		$(".addsort").hide();
		$(".changesort").show();
		var name=$(this).parent().children('td').eq(0).html();
		$("#changesortname").val(name);
		$.ajax({
			url: "SelectSortIdServlet",
			type: "POST",
			dataType: "json",
			data: {
				"name": name,
...
//	添加分类
	$('body').on('click', '#addsortok', function() {
		var name=$("#addsortname").val();
		$.ajax({
			url: "AddSortServlet",
			type: "POST",
			dataType: "json",
...
//	改变分类
	$('body').on('click', '#changesortok', function() {
		var name=$("#changesortname").val();
		$.ajax({
			url: "UpdateSortServlet",
			type: "POST",
			dataType: "json",
			data: {
				"name": name,
	...
	$('body').on('click', '#sort', function() {
		...
		$(".addsort").hide();
		$(".changesort").hide();
		$("#sort").css('background-color', 'rgb(255, 255, 255, 0)');
		$("#news").css('background-color', 'rgb(255, 255, 255, 0.3)');
		$("#add").css('background-color', 'rgb(255, 255, 255, 0.3)');
		$(".text1 div").remove();
	})
	...
	$.ajax({
		...
		success: function(data) {
			for(var i = 0; i < data.length; i++) {
				$("#table2").append("<tr><td id='name'>" +data[i].sortName + "</td> <td id='alter'>修改</td><td id='remove'>删除</td></tr>");
			}
		},
		error: function() {
			var error = "查询数据失败！";
