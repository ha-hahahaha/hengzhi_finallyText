$(document).ready(function() {
	...
	var keyword;
	// 1
	// 加载完，第一个AJAX，我把关键词给你，success data = keyword;
	// 你把关键词填到框框里
	// 2，
	// 第二个AJAX 你给我 keyword 我返回给你新闻和通知的列表 succsess 你把它加进去
	$.ajax({
		url: "TransWordServlet",
		type: "POST",
		dataType: "json",
			...
			success: function(data) {
				...
				for(var i = 0; i < data.length; i++) {
					if(title == data[i].title) {
						$("#news").append("<div class='title'>" + title + "</div><div class='remark'>" + data[i].writeTime + " " + data[i].anthor + "</div><div class='text'>" + data[i].content + "</div><div class='pre'><img src='img/shangyitiao.png'></div><div class='next'><img src='img/xiayitiao.png'></div><a href='search.html'><div class='close'>关闭窗口</div></a>");
						num = i;
					}
				}
			},
	...
	$('body').on('click', '.pre img', function() {
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(num != 0) {
					$("#news").empty();
					$("#news").append("<div class='title'>" + data[num - 1].title + "</div><div class='remark'>" + data[num - 1].writeTime + " " + data[num - 1].anthor + "</div><div class='text'>" + data[num - 1].content + "</div><div class='pre'><img src='img/shangyitiao.png'></div><div class='next'><img src='img/xiayitiao.png'></div><a href='search.html'><div class='close'>关闭窗口</div></a>");
					num = num-1;
				}
			},
			error: function() {
